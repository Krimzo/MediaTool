...
std::string hash( void const* ptr, Args&&... args )
{
    return kl::format( args..., "##", size_t( ptr ) );
}

int get_taskbar_height();
bool execute( std::wstring_view const& command );
...
int main()
{
    kl::console::set_title( "Video Handler" );
    kl::Window window{ "Video Handler" };
    ...
    GPUEncoder::ADAPTER_NAME = adapter_desc.Description;
    switch ( adapter_desc.VendorId )
    {
    case 0x10DE: GPUEncoder::DEFAULT_VENDOR = GPUVendor::NVIDIA; break;
    case 0x1002: GPUEncoder::DEFAULT_VENDOR = GPUVendor::AMD; break;
    case 0x8086: GPUEncoder::DEFAULT_VENDOR = GPUVendor::INTEL; break;
    }
    ...
    while ( window.process() )
    {
        ...
        {
            static const kl::Float4 SYSTEM_COLOR = kl::RGB{ 117, 203, 224 };
            static const kl::Float4 YTDLP_COLOR = kl::RGB{ 219, 202, 118 };
            ...
            { // yt-dlp
                im::TextColored( (ImVec4 const&) YTDLP_COLOR, "YT-DLP" );

                im::Text( "To Download" );
                im::SameLine();
                {
                    std::string temp = kl::convert_string( ytdlp_data.url );
                    if ( im::InputText( "##YTDLPToDownload", &temp ) )
                        ytdlp_data.url = kl::convert_string( temp );
                }

                bool ytdlp_has_output = ytdlp_data.output_file.has_value();
                if ( im::Checkbox( "Output##YTDLP", &ytdlp_has_output ) )
                {
                    if ( ytdlp_has_output )
                        ytdlp_data.output_file.emplace();
                    else
                        ytdlp_data.output_file.reset();
                }
                if ( ytdlp_data.output_file )
                {
                    im::SameLine();
                    if ( im::Button( "Browse##YTDLPOutputFileButton" ) )
                    {
                        if ( auto opt_path = kl::wchoose_file( true ) )
                            ytdlp_data.output_file = std::filesystem::absolute( *opt_path ).wstring();
                    }
                    im::SameLine();
                    im::Text( "Output File: %s", kl::convert_string( *ytdlp_data.output_file ).data() );
                }

                im::Checkbox( "Audio Only##YTDLP", &ytdlp_data.audio_only );

                const std::wstring ytdlp_full_command = ytdlp_data.produce();
                im::TextWrapped( "%s", kl::convert_string( ytdlp_full_command ).c_str() );

                if ( im::Button( "Download##YTDLP" ) )
                {
                    if ( ytdlp_data.url.empty() )
                        kl::print( kl::colors::RED, "YTDLP: Video link must be specified.", kl::colors::CONSOLE );
                    else
                        execute( ytdlp_full_command );
                }
            }
...
std::wstring YTDLPData::produce() const
{
...
void GPUEncoder::edit()
{
    ...
    if ( im::Checkbox( hash( this, "Nvidia" ).c_str(), &nvidia ) )
    ...
    if ( im::Checkbox( hash( this, "AMD" ).c_str(), &amd ) )
    ...
    if ( im::Checkbox( hash( this, "Intel" ).c_str(), &intel ) )
    ...
    if ( im::Checkbox( hash( this, "H264" ).c_str(), &h264 ) )
        codec_type = CodecType::H264;
    im::SameLine();
    bool hevc = codec_type == CodecType::HEVC;
    if ( im::Checkbox( hash( this, "HEVC" ).c_str(), &hevc ) )
        codec_type = CodecType::HEVC;
    im::SameLine();
    bool av1 = codec_type == CodecType::AV1;
    if ( im::Checkbox( hash( this, "AV1" ).c_str(), &av1 ) )
        codec_type = CodecType::AV1;
}

std::wstring GPUEncoder::produce() const
{
...
bool execute( std::wstring_view const& command )
{
    ...
    kl::console::set_title( kl::wformat( "Running: ", command ) );
