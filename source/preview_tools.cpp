...
void mt::preview_timestamp(fs::path const& path, Timestamp& timestamp)
{
    ...
    kl::Window window{kl::format("Timestamp Preview ", path.filename().string(), " [", timestamp.minutes, ":",
                                 timestamp.seconds, "]")};
...
{
    kl::VideoReader reader{path.wstring()};
    kl::Window window{kl::format("Video Crop Preview ", path.filename().string(), " [", timestamp.minutes, ":",
                                 timestamp.seconds, "]")};
    kl::GPU gpu{window.ptr()};
    ...
    while (window.process())
    {
                         ...
                         ImGuiWindowFlags_NoDecoration | ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoDocking))
        {
            ...
            {
                const ImVec2 mouse_pos = imgui_context->Style.WindowPadding - min_content +
                                         im::WindowPosAbsToRel(im::GetCurrentWindow(), im::GetMousePos());
