...
struct LogInfo
{
    ...
    LogInfo(std::string message) : message(std::move(message))
    {
    }

    template <typename... Args>
    LogInfo(float4 const& color, Args&&... objects)
        : color(reinterpret_cast<ImVec4 const&>(color)), message(kl::format(objects...))
    {
    }
};
...
struct Logger
{
    ...
    template <typename... Args> static void log(float4 const& color, Args&&... objects)
    {
