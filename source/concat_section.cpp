...
{
    return this->video_resolution == other.video_resolution && this->frame_rate == other.frame_rate &&
...
void mt::ConcatSection::display()
{
    ...
    DRAG_DROP_PACKAGE.dragdrop_to_imgui(
        [&](std::wstring const& path) { inputs.emplace_back().set_path(fs::absolute(path).wstring()); });
    if (to_remove)
