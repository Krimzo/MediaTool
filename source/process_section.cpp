...
std::wstring mt::ProcessSection::produce(fs::path const& input_file, MediaType& out_media_type,
                                         fs::path* outout_file) const
{
    ...
    if (image_output_ext && opt_content_type->starts_with("image"))
    {
        ...
        ffmpeg.custom_commands =
            kl::format("-vf \"scale='min(", max_image_dimension, ",iw)':min'(", max_image_dimension,
    ...
    else if (video_output_ext && opt_content_type->starts_with("video"))
    {
        ...
        ffmpeg.custom_commands =
            kl::format("-vf \"scale='min(", max_video_dimension, ",iw)':min'(", max_video_dimension,
                       ",ih)':force_original_aspect_ratio=decrease:force_divisible_by=2,fps=fps='min(",
                       max_video_framerate, ",source_fps)'\"");
...
void mt::ProcessSection::display()
{
    ...
    im::DragInt(QNAME("##MaxImageDimension"), &max_image_dimension, 0.1f, 0, 1'000'000, "%d",
                ImGuiSliderFlags_AlwaysClamp);

    im::Text("Max Video Dimension");
    im::SameLine();
    im::SetNextItemWidth(100.0f);
    im::DragInt(QNAME("##MaxVideoDimension"), &max_video_dimension, 0.1f, 0, 1'000'000, "%d",
                ImGuiSliderFlags_AlwaysClamp);

    im::Text("Max Video Framerate");
    im::SameLine();
    im::SetNextItemWidth(100.0f);
    im::DragInt(QNAME("##MaxVideoFramerate"), &max_video_framerate, 0.1f, 0, 1'000'000, "%d",
                ImGuiSliderFlags_AlwaysClamp);
    ...
    ImVec2 text_size =
        im::CalcTextSize(kl::convert_string(full_command).c_str(), nullptr, false, im::GetContentRegionAvail().x);
    ...
    im::BeginDisabled(input_dir.empty() || output_dir.empty() ||
                      fs::equivalent(input_dir, output_dir, _ignored_error) ||
                      (!image_output_ext && !audio_output_ext && !video_output_ext));
...
void mt::ProcessSection::process() const
{
    ...
    std::jthread progress_thread{[&]() { progress_window.run("Process Progress"); }};
