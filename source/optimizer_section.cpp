...
const float4 mt::OptimizerSection::COLOR = kl::RGB{255, 198, 149};
...
void mt::OptimizerSection::display()
{
    ...
    im::SetNextItemWidth(100.0f);
    im::SetCursorPosY(im::GetCursorPosY() - imgui_context->Style.FramePadding.y);
    if (im::DragFloat(QNAME("##SizeLimitsX"), &size_limits_mb.x, 0.01f, 0.0f, 1e6f, "%.3f",
                      ImGuiSliderFlags_AlwaysClamp))
        size_limits_mb.y = kl::max(size_limits_mb.y, size_limits_mb.x);
    im::SameLine();
    im::SetNextItemWidth(100.0f);
    im::SetCursorPosY(im::GetCursorPosY() - imgui_context->Style.FramePadding.y);
    if (im::DragFloat(QNAME("##SizeLimitsY"), &size_limits_mb.y, 0.01f, 0.0f, 1e6f, "%.3f",
                      ImGuiSliderFlags_AlwaysClamp))
    ...
    im::BeginDisabled(input_file.empty() || output_file.empty() || size_limits_mb.x >= size_limits_mb.y ||
                      file_size_mb(input_file) < size_limits_mb.y);
