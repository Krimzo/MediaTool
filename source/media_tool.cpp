...
bool mt::MediaTool::update()
{
    ...
    const std::string system_title_extension =
        system_unseen_count > 0 ? kl::format(" [", system_unseen_count, "]###SystemTitle") : "###SystemTitle";

    im::PushStyleVar(ImGuiStyleVar_WindowRounding, 0.0f);
    im::PushStyleVar(ImGuiStyleVar_WindowPadding, ImVec2{});
    if (im::Begin(QNAME("Main Window"), nullptr,
                      ...
                      ImGuiWindowFlags_NoScrollbar))
    {
        ...
        if (tab_button(current_section.is<SystemSection>(), kl::format("System", system_title_extension).data(),
                       button_size, SystemSection::COLOR))
