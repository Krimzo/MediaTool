...
void mt::FFMPEGSection::display()
{
    ...
    if (start_time)
    {
        ...
        im::DragInt(QNAME("##StartTimeMinutes"), &start_time->minutes, 0.05f, 0, 1'000'000, "%d",
                    ImGuiSliderFlags_AlwaysClamp);
        im::SameLine();
        im::SetNextItemWidth(100.0f);
        im::DragFloat(QNAME("##StartTimeSeconds"), &start_time->seconds, 0.01f, 0.0f, 59.999f, "%.3f",
                      ImGuiSliderFlags_AlwaysClamp);
    ...
    if (end_time)
    {
        im::SameLine();
        im::SetNextItemWidth(100.0f);
        im::DragInt(QNAME("##EndTimeMinutes"), &end_time->minutes, 0.05f, 0, 1'000'000, "%d",
                    ImGuiSliderFlags_AlwaysClamp);
        im::SameLine();
        im::SetNextItemWidth(100.0f);
        im::DragFloat(QNAME("##EndTimeSeconds"), &end_time->seconds, 0.01f, 0.0f, 59.999f, "%.3f",
                      ImGuiSliderFlags_AlwaysClamp);
        if (!input_file.empty() && kl::probe_content_type(input_file).value_or({}).starts_with("video"))
        {
            im::SameLine();
    ...
    if (std::holds_alternative<DefaultCodec>(codec))
    {
        ...
        else if (VideoCrop* video_crop = std::get_if<VideoCrop>(&codec.video_viewport))
        {
            ...
            im::SameLine();
            im::SetNextItemWidth(200.0f);
            im::DragInt2(QNAME("Top Left##VideoCrop"), &video_crop->top_left.x, 1.0f, 0, 1'000'000, "%d",
                         ImGuiSliderFlags_AlwaysClamp);
            im::Text("\t");
            im::SameLine();
            im::SetNextItemWidth(200.0f);
            im::DragInt2(QNAME("Bottom Right##VideoCrop"), &video_crop->bottom_right.x, 1.0f, 1, 1'000'000, "%d",
                         ImGuiSliderFlags_AlwaysClamp);
            if (!input_file.empty() && kl::probe_content_type(input_file).value_or({}).starts_with("video"))
            {
                im::SameLine();
    ...
    im::BeginDisabled(input_file.empty() || output_file.empty() ||
                      (start_time && end_time && start_time->total_seconds() >= end_time->total_seconds()));
    if (im::Button(QNAME("Produce"), {im::GetContentRegionAvail().x, 30.0f}))
