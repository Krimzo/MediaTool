...
bool mt::execute(HWND window, std::wstring_view const& command, bool pause)
{
    ...
    MoveWindow(GetConsoleWindow(), rect.left, rect.top, rect.right - rect.left, rect.bottom - rect.top, false);
...
void mt::auto_adjust_window_height(kl::Window const& window)
{
    ...
    const int final_cursor_y =
        kl::min((int)ImGui::GetCursorScreenPos().y, int(kl::SCREEN_SIZE.y * MAX_SCREEN_HEIGHT_PERC));
...
bool mt::input_text_multiline_hint(const char* label, const char* hint, char* buf, size_t buf_size, const ImVec2& size,
                                   ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* user_data)
{
    return im::InputTextEx(label, hint, buf, (int)buf_size, size, flags | ImGuiInputTextFlags_Multiline, callback,
                           user_data);
}

bool mt::input_text_multiline_hint(const char* label, const char* hint, std::string* str, const ImVec2& size,
                                   ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* user_data)
{
    ...
    return input_text_multiline_hint(label, hint, (char*)str->c_str(), str->capacity() + 1, size, flags,
